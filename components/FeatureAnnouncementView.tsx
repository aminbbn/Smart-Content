    ...
    return (
        <div className="space-y-8 animate-page-enter w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200/60 pb-6">
                <div>
                    <h2 className="text-3xl font-extrabold text-slate-800 tracking-tight">ایجنت معرفی محصول</h2>
                    <p className="text-slate-500 mt-2 text-lg">تولید اطلاعیه‌های جذاب برای ویژگی‌ها و محصولات جدید</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-4 sticky top-8">
                    ...
                    <div className="bg-white rounded-[2rem] shadow-card border border-slate-100 p-8 relative group">
                        ...
                        <form onSubmit={handleCreate} className="space-y-6 relative z-10">
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">نام ویژگی / محصول</label>
                                <input 
                                    required
                                    ...
                                    placeholder="مثلا: نسخه جدید اپلیکیشن موبایل"
                                    value={featureName}
                                    onChange={e => setFeatureName(e.target.value)}
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">توضیحات فنی و قابلیت‌ها</label>
                                <textarea 
                                    required
                                    ...
                                    placeholder="لیست قابلیت‌ها، مزایا و تغییرات مهم را اینجا بنویسید..."
                                    value={description}
                            ...
                            <div>
                                <label className="block text-sm font-bold text-slate-700 mb-2">نویسنده</label>
                                <WriterSelector 
                                    writers={writers}
                            ...
                            >
                                {loading ? (
                                    ...
                                    <>
                                        ...
                                        <span>تولید هوشمند اطلاعیه</span>
                                    </>
                                )}
                            </button>
                        </form>
                ...
                <div className="lg:col-span-8 space-y-6">
                    <div className="flex items-center gap-3 mb-2">
                        <h3 className="font-bold text-slate-800 text-xl">اطلاعیه‌های اخیر</h3>
                    ...
                    {items.length === 0 ? (
                        <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 p-16 flex flex-col items-center justify-center text-center hover:border-blue-300 transition-colors group">
                            ...
                            <h4 className="text-xl font-bold text-slate-700 mb-2">هنوز اطلاعیه‌ای نساخته‌اید</h4>
                            <p className="text-slate-500 max-w-md mx-auto leading-relaxed">
                                مشخصات محصول جدید خود را در فرم سمت راست وارد کنید تا هوش مصنوعی یک متن معرفی جذاب برای شما بنویسد.
                            </p>
                        </div>
                    ) : (
                                ...
                                <div key={item.id} className={`bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-sm hover:shadow-card-hover transition-all group relative animate-card-enter stagger-${index % 5}`}>
                                    <div className="absolute top-6 left-6">
                                        <span className={`px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 ${
                                            ...
                                            {item.status === 'processed' ? 'تولید شده' : 'در حال پردازش'}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-start gap-4 pr-2">
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            <div className="flex items-center gap-4 border-t border-slate-50 pt-4 mt-2">
                                                ...
                                                {item.status === 'processed' && (
                                                    <button className="text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors mr-auto flex items-center gap-1">
                                                        مشاهده متن کامل
                                                        <svg className="w-3 h-3 rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                                                    </button>
                                                )}
                                            </div>
                                        </div>
