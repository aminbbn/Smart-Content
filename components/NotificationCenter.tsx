...
export default function NotificationCenter({ align = 'left', onViewArchive }: Props) {
                ...
                >
                    <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/80">
                        <div className="flex items-center gap-2">
                            <h3 className="font-bold text-slate-800 text-lg">اعلان‌ها</h3>
                            {unreadCount > 0 && <span className="bg-red-100 text-red-600 text-[10px] px-2 py-0.5 rounded-full font-bold">{unreadCount} جدید</span>}
                        </div>
                        <button 
                            onClick={markAllRead} 
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <div className="flex-grow">
                                                <div className="flex justify-between items-start mb-1">
                                                    ...
                                                    <span className="text-[10px] text-slate-400 whitespace-nowrap mr-2 bg-white px-2 py-0.5 rounded-full border border-slate-100">
                                                ...
                                                {notif.action_text && (
                                                    <button className="text-xs bg-white border border-slate-200 text-slate-600 font-bold px-3 py-1.5 rounded-lg hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all flex items-center gap-1">
                                                        {notif.action_text}
                                                        <svg className="w-3 h-3 transform rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                                                    </button>
                                                )}
                                            </div>
                                        </div>
