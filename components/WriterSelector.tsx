                    ...
                    {selectedWriter ? (
                        <>
                            ...
                            <div className="flex flex-col items-start truncate">
                                ...
                                {selectedWriter.is_default ? (
                                    <span className="text-[10px] text-blue-600 font-bold bg-blue-50 px-1.5 rounded">پیش‌فرض سیستم</span>
                                ) : (
                                    <span className="text-[10px] text-slate-400">نویسنده</span>
                                )}
                            </div>
                        </>
                    ) : (
                        <>
                            ...
                            <div className="flex flex-col items-start">
                                <span className="font-bold text-slate-800 text-sm">انتخاب هوشمند (خودکار)</span>
                                <span className="text-[10px] text-slate-400">سیستم بهترین نویسنده را انتخاب می‌کند</span>
                            </div>
                        </>
                    )}
                </div>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-100 z-50 overflow-hidden animate-slide-in max-h-72 overflow-y-auto custom-scrollbar">
                    ...
                    >
                        ...
                        <div className="flex flex-col">
                            <span className="font-bold text-slate-800 text-sm">انتخاب هوشمند (خودکار)</span>
                            <span className="text-[10px] text-slate-400">بر اساس موضوع و محتوا</span>
                        </div>
                        {selectedId === '' && <svg className="w-5 h-5 text-blue-600 mr-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
                    </div>
                        ...
                        >
                            ...
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center gap-2">
                                     ...
                                     {writer.is_default === 1 && <span className="text-[9px] bg-slate-100 text-slate-500 px-1.5 rounded border border-slate-200">پیش‌فرض</span>}
                                </div>
                            </div>
                            {Number(selectedId) === writer.id && <svg className="w-5 h-5 text-blue-600 mr-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
                        </div>
                    ))}
                </div>
            )}
